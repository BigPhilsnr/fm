...
import 'package:fmanager/constants/constants.dart';
import 'package:fmanager/model/common.dart';
import 'package:fmanager/model/doctype_response.dart';
import 'package:fmanager/services/api/dio_api.dart';

// import 'package:fbase/core/services/api/dio_api.dart';
...
class OfflineStorage {
  static var storage = Get.put(StorageService()).getHiveBox('offline');
  ...
  static putAllItems(Map data, [bool isIsolate = false]) async {
    ...
    if (isIsolate) {
      var runBackgroundTask = await Get.put(StorageService())
  ...
  static Future<Map> storeDoctypeMeta(String doctype) async {
    var cache = {};
    try {
      var response = await DioApi().getDoctype(doctype);
  ...
  static Future<Map> storeLinkField(String doctype) async {
    var cache = {};
    try {
      var linkData = await DioApi().searchLink(
        doctype: doctype,
        pageLength: 9999,
        refDoctype: '',
        txt: '',
  ...
  static Future<Map> storeDocListAndDoc(String doctype) async {
    ...
    try {
      var docMeta = await DioApi().getDoctype(doctype);

      var docList = await DioApi().fetchList(
        doctype: doctype,
  ...
  static Future<Map> storeDoc(String doctype, String docName) async {
    var cache = {};
    try {
      var docForm = await DioApi().getdoc(doctype, docName);
  ...
  static Future<bool> storeApiResponse() async {
    var storeApiResponse =
        await Get.put(StorageService()).getSharedPrefBoolValue(
      "storeApiResponse",
    );
  ...
  static Future<DoctypeResponse> getMeta(String doctype) async {
    try {
      ...
      if (isOnline && cachedMeta["data"] == null) {
        metaResponse = await DioApi().getDoctype(doctype);
      } else {
        var cachedMeta = getItem('${doctype}Meta');
