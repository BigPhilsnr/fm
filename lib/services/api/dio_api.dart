...
import 'package:fmanager/model/common.dart';
import 'package:fmanager/model/get_doc_response.dart';
import 'package:fmanager/model/get_versions_response.dart';
import 'package:fmanager/model/group_by_count_response.dart';
import 'package:fmanager/model/login_request.dart';
import 'package:fmanager/model/stats/DashStat.dart';
import 'package:fmanager/model/system_settings_response.dart';
import 'package:fmanager/model/upload_file_response.dart';
...
class DioApi {
  Future<LoginResponse> login(LoginRequest loginRequest) async {
    try {
      ...
      throw ErrorResponse(statusMessage: "Unkown Error");
    }
  }

  Future<DeskSidebarItemsResponse> getDeskSideBarItems() async {
    try {
      var response = await Get.find<Dio>().post(
      ...
      if (response.statusCode == 417) {
        response = await Get.find<Dio>().post(
      ...
      if (e is DioError) {
        ...
        if (error is SocketException) {
          ...
          throw ErrorResponse(statusMessage: "Unkown Error Occured");
        }
      } else {
        throw e;
      }
  ...
  Future<DesktopPageResponse> getDesktopPage(String module) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw ErrorResponse(
          statusCode: response.statusCode,
          statusMessage: response.statusMessage ??= " No Error Message",
        );
      } else {
        throw ErrorResponse();
      }
  ...
  Future<DoctypeResponse> getDoctype(String doctype) async {
    ...
    try {
      ...
      if (response.statusCode == HttpStatus.ok) {
        ...
        throw ErrorResponse(
          statusCode: response.statusCode,
          statusMessage: response.statusMessage ??= "Error Mesage",
        );
      } else {
        throw ErrorResponse(
          statusMessage: response.statusMessage ??= "Error Mesage",
      ...
      if (e is DioError) {
        ...
        if (error is SocketException) {
          ...
          throw ErrorResponse(statusMessage: "Unkonw");
        }
      } else {
        throw ErrorResponse();
      }
  ...
  Future<List> fetchList({
    ...
    try {
      ...
      if (response.statusCode == HttpStatus.ok) {
        ...
        //i remove await OfflineStorage.storeApiResponse()
  ...
  Future<List> fetchLedger({
    ...
    try {
      ...
      if (response.statusCode == HttpStatus.ok) {
        var l = response.data["message"]["result"];
        var newL = [];
        ...
        if (offset == 0) {
          print("STORING DATA +++++++++++++++++++++++++++++++++++++");
          await OfflineStorage.putItem('generalLedger', newL);
        }
  ...
  Future postComment(
    ...
    final response = await Get.find<Dio>().post(
        ...
        data: queryParams,
        options:
            DioPackage.Options(contentType: Headers.formUrlEncodedContentType));
    if (response.statusCode == 200) {
    } else {
  ...
  Future submitDoc(Map<String, String> data) async {
    var response = await Get.find<Dio>().post(
      '/method/practice_manager.api.dasbhoard.submit',
      data: "doc=${Uri.encodeComponent(json.encode(data))}",
      options: Options(
        contentType: Headers.formUrlEncodedContentType,
      ),
    );
    ...
    print(response);
    if (response.statusCode == 200) {
      return Stats.fromJson(response.data["message"]);
    } else {
      throw ErrorResponse(
          statusCode: response.statusCode,
          statusMessage: response.statusMessage ?? "Unknown Error occurred");
    }
  }

  Future getDashStats() async {
    var response = await Get.find<Dio>().post(
      '/method/practice_manager.api.dasbhoard.cash',
      data: {},
      options: Options(
        contentType: Headers.formUrlEncodedContentType,
      ),
  ...
  Future getCompany() async {
    var response = await Get.find<Dio>().post(
      '/method/practice_manager.api.dasbhoard.get_company',
      data: {},
      options: Options(
        contentType: Headers.formUrlEncodedContentType,
      ),
  ...
  Future getValue(
      String doctype, String fieldName, Map<String, dynamic> filters) async {
    var response = await Get.find<Dio>().post(
  ...
  Future deleteComment(String name) async {
    ...
    final response = await Get.find<Dio>().post('/method/frappe.client.delete',
        data: queryParams,
        options: Options(
          contentType: Headers.formUrlEncodedContentType,
        ));
  ...
  Future<List<UploadedFile>> uploadUserFiles({
    required String doctype,
    ...
    for (FrappeFile frappeFile in files) {
      ...
      DioPackage.FormData formData = DioPackage.FormData.fromMap({
        "file": await DioPackage.MultipartFile.fromFile(
  ...
  Future submitDocs(Map data) async {
    try {
      final response = await Get.find<Dio>().post(
        '/method/frappe.desk.form.save.savedocs',
        data: "doc=${Uri.encodeComponent(json.encode(data))}&action=Submit",
        options: Options(
          contentType: Headers.formUrlEncodedContentType,
        ),
      );
